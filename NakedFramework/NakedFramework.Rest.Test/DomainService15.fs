...
module NakedObjects.Rest.Test.DomainService15
...
let GetService(api : RestfulObjectsControllerBase) = 
    ...
    let result = api.GetService(sName)
...
let GetWithActionService(api : RestfulObjectsControllerBase) = 
    ...
    let expected = 
        [ TProperty(JsonPropertyNames.ServiceId, TObjectVal(sName))
          ...
          TProperty(JsonPropertyNames.Links, 
                    TArray([ TObjectJson(makeGetLinkProp RelValues.Self (sprintf "services/%s" sName) RepresentationTypes.Object oType)
