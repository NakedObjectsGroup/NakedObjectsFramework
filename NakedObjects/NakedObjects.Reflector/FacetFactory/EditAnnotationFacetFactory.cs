...
public sealed class EditAnnotationFacetFactory : DomainObjectFacetFactoryProcessor, IAnnotationBasedFacetFactory {
    ...
    public override IImmutableDictionary<string, ITypeSpecBuilder> Process(IReflector reflector, MethodInfo method, IMethodRemover methodRemover, ISpecificationBuilder specification, IImmutableDictionary<string, ITypeSpecBuilder> metamodel) {
        void AddFacet(IDictionary<ParameterInfo, PropertyInfo> matches) => FacetUtils.AddFacet(new EditPropertiesFacet(matches.Values.Select(p => p.Name).ToArray()), specification);
    ...
    public override IImmutableDictionary<string, ITypeSpecBuilder> ProcessParams(IReflector reflector, MethodInfo method, int paramNum, ISpecificationBuilder specification, IImmutableDictionary<string, ITypeSpecBuilder> metamodel) {
        void AddFacet(IDictionary<ParameterInfo, PropertyInfo> matches) {
            ...
            if (matches.ContainsKey(thisParameter)) {
                ...
                FacetUtils.AddFacet(new ActionDefaultsFacetViaProperty(property, defaultFacet, Logger<ActionDefaultsFacetViaProperty>()), specification);
            }
        }
