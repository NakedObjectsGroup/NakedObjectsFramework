...
type DomainTests() = class      
    ...
    override x.MenuServices 
        with get() : IServicesInstaller  =
            ...
            box (new ServicesInstaller([|(box testService); (box xmlService); (box transformService) |])) :?> IServicesInstaller
    ...
    member x.TestService = 
        x.GetTestService("Test Objects")    

    member x.TransformService = 
        x.GetTestService("Transform Repository")  
    ...
    member x.SimpleTestObject() = 
        x.TestService.GetAction("New Instance").InvokeReturnObject().NakedObject.GetDomainObject<TestObject>()
    ...
    member x.TransFormFullObject() = 
        x.TransformService.GetAction("Transform Full").InvokeReturnObject().NakedObject.GetDomainObject<One.TransformFull>()
    ...
    member x.stylesheet1 = 
        @"<xsl:stylesheet xmlns:xsl=""http://www.w3.org/1999/XSL/Transform"" version=""1.0"" xmlns:app=""http://www.nakedobjects.org/ns/app/Snapshot.Xml.Test.Two/TransformFull"">
    ...
    member x.CompareElementValues (elem1 : XElement) (elem2 : XElement) = 
        ...
        if (text1 |> Seq.isEmpty && text2 |> Seq.isEmpty) then 
            true
        elif (not(text1 |> Seq.isEmpty) && not(text2 |> Seq.isEmpty)) then 
            (text1 |> Seq.head).Value = (text2 |> Seq.head).Value
        else 
            false
