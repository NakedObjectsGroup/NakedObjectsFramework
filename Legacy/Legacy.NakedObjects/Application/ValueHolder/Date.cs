...
namespace Legacy.NakedObjects.Application.ValueHolder {
    public class Date : Magnitude {
        ...
        public Date(DateTime date, Action<DateTime> callback) : this(date) => BackingField = callback;
        ...
        public virtual void add(int years, int months, int days) {
            setValue(date.AddYears(years).AddMonths(months).AddDays(days));
        }

        public virtual Calendar calendarValue() => throw
            //if (this.isNull)
        ...
        public virtual DateTime? dateValue() => isNull ? null : date;

        public override bool Equals(object obj) {
            return obj is Date date1 ? date1.date.Equals(date) : base.Equals(obj);
        }

        public virtual int getDay() => date.Day;

        public virtual int getMonth() => date.Month;

        public virtual int getYear() => date.Year;

        public override bool isEmpty() => isNull;

        public override bool isEqualTo(Magnitude date) => throw
            //if (!(date is Date))
        ...
        public override bool isLessThan(Magnitude date) => throw
            //if (!(date is Date))
            //  throw new IllegalArgumentException("Parameter must be of type Time");
            //return !this.isNull && !date.isEmpty() && this.date.before(((Date) date).date);
            new NotImplementedException();

        public virtual long longValue() => date.Ticks;
            ...
            if (dateString == "") {
                ...
                if (str == "today" || str == "now") {
                    ...
                    if (str.StartsWith("+")) {
                        ...
                        setValue(DateTime.Parse(dateString));
                        //int length = 5;
        ...
        public virtual void reset() => today();
        ...
        private bool sameAs(Date @as, int field) => throw
        ...
        public virtual bool sameDayAs(Date @as) => sameAs(@as, 6);

        public virtual bool sameMonthAs(Date @as) => sameAs(@as, 2);

        public virtual bool sameWeekAs(Date @as) => sameAs(@as, 3);

        public virtual bool sameYearAs(Date @as) => sameAs(@as, 1);

        public override string asEncodedString() => throw
            //if (this.isEmpty())
            //  return "NULL";
        ...
        public virtual void setValue(Date date) {
            if (date is null || date.isEmpty()) {
                ...
                setValue(new DateTime(date.longValue()));
            }
        }

        public virtual void setValue(int year, int month, int day) {
            setValue(new DateTime(year, month, day));
        }
