...
module NakedObjects.Angular.Gemini {
    ...
    app.service("urlManager", function ($routeParams: INakedObjectsRouteParams, $location: ng.ILocationService) {
        ...
        helper.isList = () => {
            return $location.search()["action1"]; // because a list is generated by an action
        }
        helper.isMenuOpen = () => {
            return $location.search()["menu1"]; //|| $location.search()["actions1"]; //latter is menu on an object
        }
        helper.isActionOpen = () => {
            return $location.search()["dialog1"];
        }
        helper.isCollectionOpen = () => {
            return $location.search()["collection1"];
        }
        helper.isTable = () => {
            return $location.search()["table"]; //todo: is this correct -  table is a value not attribute?
        }
        helper.isEdit = () => {
            return $location.search()["edit1"];
        }
    });
