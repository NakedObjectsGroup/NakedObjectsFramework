...
public static class MenusApi {
    public static IEnumerable<Link> GetValue(this Menus menusRepresentation) => menusRepresentation.GetMandatoryProperty(JsonConstants.Value).ToLinks();
    ...
    public static async Task<DomainObject?> GetMenu(this Menus menusRepresentation, string menuId, InvokeOptions options) =>
        menusRepresentation.GetMenuLink(menuId) is { } l ? new DomainObject(await ApiHelpers.GetResourceAsync(l, options)) : null;
}