    ...
    let expected = 
        [ TProperty(JsonPropertyNames.Id, TObjectVal(pid))
          ...
          TProperty(JsonPropertyNames.Links, 
                    TArray([ TObjectJson(makeGetLinkProp RelValues.Self purl RepresentationTypes.ActionDescription "")
    ...
    let expected = 
        [ TProperty(JsonPropertyNames.Id, TObjectVal(pid))
          ...
          TProperty(JsonPropertyNames.Parameters, 
                    TArray([ TObjectJson(makeGetLinkProp RelValues.ActionParam p1url RepresentationTypes.ActionParamDescription "")
...
let GetActionTypeObjectContributedOnContributer(api : RestfulObjectsControllerBase) = 
    ...
    let expected = 
        [ TProperty(JsonPropertyNames.Id, TObjectVal(pid))
          ...
          TProperty
              (JsonPropertyNames.Parameters, TArray([ TObjectJson(makeGetLinkProp RelValues.ActionParam p1url RepresentationTypes.ActionParamDescription "") ]))
          TProperty(JsonPropertyNames.Extensions, TObjectJson([]))
          TProperty(JsonPropertyNames.Links, 
                    TArray([ TObjectJson(makeGetLinkProp RelValues.Self purl RepresentationTypes.ActionDescription "")
                             TObjectJson(makeGetLinkProp RelValues.Up ourl RepresentationTypes.DomainType "")
...
let NotAcceptableActionType(api : RestfulObjectsControllerBase) = 
    ...
    try 
        api.Request <- msg
        api.GetActionType(oType, pid, args) |> ignore
