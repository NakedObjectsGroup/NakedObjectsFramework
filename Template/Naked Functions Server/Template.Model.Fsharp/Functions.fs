...
module Student_MenuFunctions =
    ...
    let AllStudents (context : IContext) =
        context.Instances<Student>()

    let FindStudentByName (name : string) (context : IContext) = 
        context.Instances<Student>().Where(fun c -> c.FullName.ToUpper().Contains(name.ToUpper()));

    let FindStudentById (id : int) (context : IContext)  = 
        context.Instances<Student>().FirstOrDefault(fun c -> c.Id = id);