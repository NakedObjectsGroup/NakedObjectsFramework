...
public static class LinkApi {
    ...
    public static IDictionary<string, object?>? GetArgumentsMembers(this Link linkRepresentation) => linkRepresentation.GetArgumentsAsJObject()?["members"]?.Children().Cast<JProperty>().ToDictionary(p => p.Name, p => p.MapToObject());
}