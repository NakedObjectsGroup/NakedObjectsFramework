...
namespace RestfulObjects.Test.EndToEnd {
    public abstract class AbstractActionInvokeGet : AbstractAction {
        ...
        protected void DoWithFormalDomainModel() {
            string url = BaseUrl + "AnActionReturnsQueryable" + @"/invoke" + JsonRep.FormalDomainModeAsQueryString;
            Helpers.TestResponse(url, FilePrefix + "WithFormalDomainModel" + "-Invoke");
        }
        ...
        protected void DoScalarParametersAsQueryString() {
            ...
            string url = BaseUrl + actionName + @"/invoke" + Parm1Is100Parm2IsFredAsQueryString();
            Helpers.TestResponse(url, FilePrefix + actionName);
        }

        protected void DoSyntacticallyMalformedQueryString() {
            const string actionName = Domain.AnActionReturnsQueryableWithScalarParameters;
            string url = BaseUrl + actionName + @"/invoke" + "?parm1=100&param2=fred"; //mis-named parameter
            Helpers.TestResponse(url, null, null, Methods.Get, Codes.SyntacticallyInvalid);
        ...
        protected void DoSemanticallyMalformedQueryString() {
            const string actionName = Domain.AnActionReturnsQueryableWithScalarParameters;
            string url = BaseUrl + actionName + @"/invoke" + "?parm1=foo&parm2=fred"; //i.e. a string that should be a number
            Helpers.TestResponse(url, null, null, Methods.Get, Codes.SyntacticallyInvalid);
        }
