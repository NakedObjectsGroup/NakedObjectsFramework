...
let CodeFirstCeConfig =    
    let c = new CodeFirstEntityContextConfiguration()
    c.DbContext <- fun () -> upcast new CodeFirstContext() 
    c
...
let CanCreateEntityPersistor codeOnlyPersistor =
    Assert.IsNotNull(codeOnlyPersistor)
    
let CanGetInstancesGeneric codeOnlyPersistor =      
     GetInstancesGenericNotEmpty<Product> codeOnlyPersistor

let CanGetInstancesByType codeOnlyPersistor =      
     GetInstancesByTypeNotEmpty<Product> codeOnlyPersistor 
    
let CanGetInstancesIsProxy codeOnlyPersistor =    
    GetInstancesReturnsProxies<Product> codeOnlyPersistor
   
let CanCreateTransientObject codeOnlyPersistor  =     
    CanCreateTransientObject<Product> codeOnlyPersistor

let CanSaveTransientObjectWithScalarProperties codeOnlyPersistor  =
    CanSaveTransientObject codeOnlyPersistor (productSetter codeOnlyPersistor)
...
let GetNextAddressID codeOnlyPersistor =
...
let CanGetNonPersistedClass codeOnlyPersistor =   
    try
        ...
        Assert.Fail()
    with
        | expected -> Assert.IsInstanceOf(typeof<NakedObjectApplicationException>, expected)
