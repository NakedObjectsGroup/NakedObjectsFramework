...
namespace Template.Test.Helpers {
    public static class ActionHelpers {
        public static JObject InvokeAction(this AbstractRestTest tc, Key key, string actionName) => tc.InvokeAction(key, actionName, new List<(string name, string value)>());

        public static JObject InvokeAction(this AbstractRestTest tc, Key key, string actionName, IList<(string name, string value)> parameters) {
            tc.StartServerTransaction();
            var api = TestHelpers.GetController(tc).AsPost();
