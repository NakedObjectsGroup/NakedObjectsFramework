...
namespace NakedObjects.Reflector.TypeFacetFactory {
    public sealed class IntValueTypeFacetFactory : ValueUsingValueSemanticsProviderFacetFactory {
        ...
        public override IImmutableDictionary<string, ITypeSpecBuilder> Process(IReflector reflector, IClassStrategy classStrategy, Type type, IMethodRemover methodRemover, ISpecificationBuilder specification, IImmutableDictionary<string, ITypeSpecBuilder> metamodel) {
            ...
            var (oSpec, mm) = reflector.LoadSpecification<IObjectSpecImmutable>(IntValueSemanticsProvider.AdaptedType, classStrategy, metamodel);
            AddValueFacets(new IntValueSemanticsProvider(oSpec, specification), specification);
            return mm;
        }
    }
}