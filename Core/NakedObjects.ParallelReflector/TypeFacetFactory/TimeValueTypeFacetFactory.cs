...
namespace NakedObjects.ParallelReflect.TypeFacetFactory {
    public sealed class TimeValueTypeFacetFactory : ValueUsingValueSemanticsProviderFacetFactory {
        ...
        public override IImmutableDictionary<string, ITypeSpecBuilder> Process(IReflector reflector, IClassStrategy classStrategy, Type type, IMethodRemover methodRemover, ISpecificationBuilder specification, IImmutableDictionary<string, ITypeSpecBuilder> metamodel) {
            ...
            var (oSpec, mm) = reflector.LoadSpecification<IObjectSpecImmutable>(TimeValueSemanticsProvider.AdaptedType, classStrategy, metamodel);
            AddValueFacets(new TimeValueSemanticsProvider(oSpec, specification), specification);
            return mm;
        }
    }
}