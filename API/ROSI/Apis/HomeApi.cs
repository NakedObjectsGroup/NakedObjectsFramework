...
public static class HomeApi {
    ...
    public static async Task<User> GetUser(this Home homeRepresentation, InvokeOptions options) => new(await ApiHelpers.GetResourceAsync(homeRepresentation.GetUserLink(), options));

    public static async Task<Version> GetVersion(this Home homeRepresentation, InvokeOptions options) => new(await ApiHelpers.GetResourceAsync(homeRepresentation.GetVersionLink(), options));

    public static async Task<Services?> GetServices(this Home homeRepresentation, InvokeOptions options) => homeRepresentation.GetServicesLink() is { } l ? new Services(await ApiHelpers.GetResourceAsync(l, options)) : null;

    public static async Task<Menus?> GetMenus(this Home homeRepresentation, InvokeOptions options) => homeRepresentation.GetMenusLink() is { } l ? new Menus(await ApiHelpers.GetResourceAsync(l, options)) : null;
}