...
(function (NakedObjects) {
    ...
    NakedObjects.app.service("viewModelFactory", function ($q, $timeout, $location, $filter, $cacheFactory, repLoader, color, context, mask, urlManager, focusManager, navigation, clickHandler, commandFactory, $rootScope, $route) {
        ...
        viewModelFactory.getItems = function (links, populateItems, routeData, listViewModel) {
            ...
            if (populateItems) {
                ...
                getExtensions().then(function (ext) {
                    _.forEach(items, function (itemViewModel) {
                        var tempTgt = itemViewModel.link.getTarget();
                        context.getObject(routeData.paneId, tempTgt.getDtId().dt, tempTgt.getDtId().id, routeData.interactionMode).
                            ...
                            if (!listViewModel.header) {
                                var propertiesHeader = _.map(itemViewModel.target.properties, function (property) { return property.title; });
        ...
        viewModelFactory.collectionViewModel = function (collectionRep, routeData) {
            ...
            else if (state === NakedObjects.CollectionViewState.List || state === NakedObjects.CollectionViewState.Table) {
                context.getCollectionDetails(collectionRep).then(function (details) {
        ...
        viewModelFactory.tableRowViewModel = function (objectRep, routeData, idsToShow) {
            var tableRowViewModel = new NakedObjects.TableRowViewModel();
            var properties = idsToShow ? _.pick(objectRep.propertyMembers(), idsToShow) : objectRep.propertyMembers();
            tableRowViewModel.properties = _.map(properties, function (property, id) { return viewModelFactory.propertyViewModel(property, id, null, routeData.paneId, function () { return {}; }); });
            ...
            if (cvm == null) {
                ...
                cvm.renderList = function (routeData) {
                    if (cvm.message) {
                        ...
                        var listPromise = context.getListFromMenu(1, routeData.menuId, routeData.actionId, routeData.actionParams, routeData.page, routeData.pageSize);
