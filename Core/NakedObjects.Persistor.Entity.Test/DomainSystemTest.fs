...
type DomainSystemTests() =
    ...
    [<Test>]
    member x.GetService() = 
        let srService = x.NakedObjectsFramework.Services.GetService("repository#AdventureWorksModel.ScrapReason")
    ...
    member x.CheckItemIdentities() = 
       ...
       let getSrByKey (key : Int16) =  ctx.Persistor.Instances<ScrapReason>() |> Seq.filter (fun s -> s.ScrapReasonID = key) |> Seq.head
