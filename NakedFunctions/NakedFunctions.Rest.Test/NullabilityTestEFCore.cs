...
public class NullabilityTestEFCore : NullabilityTestEF6 {
    protected override Action<NakedFrameworkOptions> AddPersistor => builder => { builder.AddEFCorePersistor(); };

    protected override void RegisterTypes(IServiceCollection services) {
        base.RegisterTypes(services);
        services.AddDbContext<DbContext, EFCoreNullabilityDbContext>();
    }

    protected override void CleanUpDatabase() {
        new EFCoreNullabilityDbContext().Delete();
    }

    protected override void CreateDatabase() {
        new EFCoreNullabilityDbContext().Create();
    }
}