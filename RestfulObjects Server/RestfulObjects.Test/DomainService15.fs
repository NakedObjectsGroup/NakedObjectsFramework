...
let GetContributorService(api : RestfulObjectsControllerBase) = 
    ...
    let expected = 
        [ TProperty(JsonPropertyNames.ServiceId, TObjectVal(sName))
          ...
          TProperty
              (JsonPropertyNames.Members, 
               TObjectJson([ TProperty("ANonContributedAction", TObjectJson(makeServiceActionMemberNoParms "ANonContributedAction" sName mst)) ]))
          TProperty(JsonPropertyNames.Extensions, 
                    TObjectJson([ TProperty(JsonPropertyNames.DomainType, TObjectVal(sName))
...
let GetServiceFormalOnly(api : RestfulObjectsControllerBase) = 
    ...
    let expected = 
        [ TProperty(JsonPropertyNames.ServiceId, TObjectVal(sName))
          ...
          TProperty(JsonPropertyNames.Links, 
                    TArray([ TObjectJson
                                 (makeLinkPropWithMethodAndTypes "GET" RelValues.Self (sprintf "services/%s" sName) RepresentationTypes.Object sName "" false)
                             TObjectJson(makeGetLinkProp RelValues.DescribedBy (sprintf "domain-types/%s" sName) RepresentationTypes.DomainType "") ]))
                    ...
                    TObjectJson([ TProperty
                                  ...
                                  TProperty
                                      ("AnActionReturnsScalar", 
                                       TObjectJson(makeActionMemberNumber "services" "AnActionReturnsScalar" oid "An Action Returns Scalar" "" "integer" []))
                                  ...
                                  TProperty
                                      ("AnActionValidateParameters", 
                                       ...
                                       TObjectJson
                                           (makeActionMemberNumber "services" "AnActionValidateParameters" oid "An Action Validate Parameters" "" "integer" 
                                                [ p37; p38 ]))
