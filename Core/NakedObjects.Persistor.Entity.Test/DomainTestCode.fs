...
let First<'t when 't : not struct> persistor = 
    First<'t> persistor

let Second<'t when 't : not struct> persistor  = 
    Second<'t> persistor
...
let CanPersistingPersistedCalledForCreateInstance persistor  =
     PersistingPersistedCalledForCreateInstance persistor CanSaveTransientObjectWithScalarProperties
...
let CanCreateManyToMany (persistor : EntityObjectStore) =
    ...
    let getPandSO() =           
        prs |> Seq.filter (fun p -> (sos |> Seq.exists (fun so -> noEntryFor p so)))
...
let CanGetObjectBySingleKey persistor = 
    CanGetObjectByKey<ScrapReason> persistor [|box 16s|]  
    
let CanGetObjectByMultiKey persistor = 
    CanGetObjectByKey<CustomerAddress> persistor [|box 1; box 832|]  
    
let CanGetObjectByStringKey persistor = 
    CanGetObjectByKey<CountryRegion> persistor [|box "AU"|]  
    
let CanGetObjectByDateKey persistor = 
    let datetime = new DateTime(2001, 7, 1)
    CanGetObjectByKey<SalesPersonQuotaHistory> persistor [|box 268; box datetime |]  
...
let CanRemoteResolve (persistor : EntityObjectStore) =
   ...
   if nakedObj.ResolveState.IsResolvable() then        
       persistor.ResolveImmediately (nakedObj)      
...
let CanDetectConcurrency (persistor : EntityObjectStore) = 
    ...
    try
        ...
        Assert.Fail()
    with
        | expected -> Assert.IsInstanceOf(typeof<ConcurrencyException>, expected)        
    Assert.AreEqual(sr1.Name, sr2.Name)
    sr1.Name <- origName
    SaveAndEndTransaction persistor sr1 
